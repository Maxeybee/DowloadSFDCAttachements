...
IF "%JAVA_HOME%" == "" (
    ...
    IF NOT EXIST "%JAVA_HOME%" (
        ...
        "%JAVA_HOME%\java" -cp "%EXE_PATH%\..\dataloader-%DATALOADER_VERSION%-uber.jar" com.salesforce.dataloader.process.DataLoaderRunner salesforce.config.dir=%1 run.mode=batch %PROCESS_OPTION%
    )
)
